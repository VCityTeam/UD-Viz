...
export function extrudeHeightMap(heightMap, chunkSize = Infinity) {
  if (!heightMap.length) {
  ...
  if (chunkSize != Infinity) {
    while (chunkSize < voxelGeometries.length) {
      const chunk = voxelGeometries.slice(0, chunkSize);
      voxelGeometries = voxelGeometries.slice(chunkSize);
      result.push(mergeGeometries(chunk, false)); // do not useGroup
    }
  }
