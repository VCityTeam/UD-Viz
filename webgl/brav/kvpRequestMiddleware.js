...
//	/ressources/paris/LOD0/tiles
	...
	//	var resJson=tempfunc.buildJsonFromTuile(req.query.TILEROW,req.query.TILECOL,req.query.TILELEVEL);
		...
		}else if(parseInt(req.query.TILELEVEL)!= 2 ){
			var err = {};
			err.code=501;
			err.API=true;
			err.msg='Not availlable yet';
			next(err);
		}else{
			if(req.query.LAYER == 'build'){
				stream = fs.createReadStream('./ressources/paris/LOD2/build/ZoneAExporter_'+req.query.TILEROW+'_'+req.query.TILECOL+'.json');
			}else{
				var err = {};
				err.code=404;
				err.API=true;
...
exports.mandatoryParamValidator = function (req, res, next){
	...
	if(!req.query.hasOwnProperty('X')){
		code = 400;
		msg += ' Missing mandatory argument X;';
	}else if(req.query.X testfaux){
		code =400;
		msg += ' Incorect Value for parameter X: '+req.query.X+' Expected Value: TO DO ';
	}
	*/
